...
namespace Balance.Service.RealtimeData
{
    public class SetRealtimeData
    {
        ...
        private void UpdateLocalData(object myUpdateParemeters)
        {
            try
            {
                ...
                while (_IsClosing == false)
                {
                    ...
                    LocalStringDataGroup m_LocalStringDataGroupTemp = _LocalWebServiceBuffer[m_DataGroupIndex].PopStringDataGroup();
                    ...
                    if (m_LocalStringDataGroupTemp != null)
                    {
                        byte[] m_StringTagName = DataCompression.Function_DefaultCompressionArray.CompressString(m_LocalStringDataGroupTemp.LocalStringTagName.ToArray());
                        byte[] m_StringTagValue = DataCompression.Function_DefaultCompressionArray.CompressString(m_LocalStringDataGroupTemp.LocalStringTagValue.ToArray());
                        try
                        {
                            m_LocalRealtimeDataService.SetStringDataCompress(m_OrganizationId, m_StringTagName, m_StringTagValue, ValidWriteString);
                        }
                        catch
                        {
                        }
                        ...
                        for (int i = 0; i < _LocalWebServiceBuffer.Count; i++)
                        {
                            m_Count = m_Count + _LocalWebServiceBuffer[i].StringCount;
                        }
                        onBufferChange("LocalStringData", m_Count);
                    }
        ...
        private void UpdateRemoteData(object myUpdateParemeters)
        {
            try
            {
                ...
                while (_IsClosing == false)
                {
                    ...
                    RemoteStringDataGroup m_RemoteStringDataGroupTemp = _RemoteWebServiceBuffer[m_DataGroupIndex].PopStringDataGroup();
                    ...
                    /////////////////////增加字符串数据///////////////
                    if (m_RemoteStringDataGroupTemp != null)
                    {
                        byte[] m_StringTagName = DataCompression.Function_DefaultCompressionArray.CompressString(m_RemoteStringDataGroupTemp.RemoteStringTagName.ToArray());
                        byte[] m_StringTagValue = DataCompression.Function_DefaultCompressionArray.CompressString(m_RemoteStringDataGroupTemp.RemoteStringTagValue.ToArray());
                        try
                        {
                            m_RemoteRealtimeDataService.SetStringDataCompress(m_OrganizationId, m_StringTagName, m_StringTagValue, ValidWriteString);
                        }
                        catch
                        {
                        }
                        ...
                        for (int i = 0; i < _RemoteWebServiceBuffer.Count; i++)
                        {
                            m_Count = m_Count + _RemoteWebServiceBuffer[i].StringCount;
                        }
                        onBufferChange("RemoteStringData", m_Count);
                    }
                    Thread.Sleep(MinQueryInterval);
                }
            }
        ...
        private void GetRealTimeDataFromDB(string[] myDataTableName, string myPrefixWord, int myDataGroupIndex)
        {
            ...
            Dictionary<string, string> m_DCSUpdateTime = new Dictionary<string, string>();       //新增取DCS数据采集系统时间
            ...
            if (_LocalWebServiceBuffer[myDataGroupIndex].CanWrite == true || _RemoteWebServiceBuffer[myDataGroupIndex].CanWrite == true)            //只有大于0才执行
            {
                for (int i = 0; i < myDataTableName.Length; i++)
                {
                    ...
                    if (m_RealtimeValueTable != null)
                    {
                        if (m_RealtimeValueTable.Rows.Count > 0)
                        {
                            ////////////以下是增加取DCS更新时间///////////
                            try
                            {
                                string m_DataBaseName = myDataTableName[i].Substring(0, myDataTableName[i].IndexOf('.'));
                                if (!m_DCSUpdateTime.ContainsKey(m_DataBaseName))
                                {
                                    m_DCSUpdateTime.Add(m_DataBaseName, ((DateTime)m_RealtimeValueTable.Rows[0]["vDate"]).ToString("yyyy-MM-dd HH:mm:ss"));
                                }
                            }
                            catch
                            {
                ...
                {
                    if (_LocalWebServiceBuffer[myDataGroupIndex].CanWrite == true)
                    {
                        _LocalWebServiceBuffer[myDataGroupIndex].SetDigitalDataGroup(m_DigitalTagName, m_DigitalTagValue);
                    }
                    if (_RemoteWebServiceBuffer[myDataGroupIndex].CanWrite == true)
                    {
                ...
                if (m_DCSUpdateTime.Count > 0)             //传送DCS更新时间
                {
                    if (_LocalWebServiceBuffer[myDataGroupIndex].CanWrite == true)
                    {
                        _LocalWebServiceBuffer[myDataGroupIndex].SetStringDataGroup(m_DCSUpdateTime.Keys.ToList(), m_DCSUpdateTime.Values.ToList());
                    }
                    if (_RemoteWebServiceBuffer[myDataGroupIndex].CanWrite == true)
                    {
                        _RemoteWebServiceBuffer[myDataGroupIndex].SetStringDataGroup(m_DCSUpdateTime.Keys.ToList(), m_DCSUpdateTime.Values.ToList());
                    }
                }
            }
            else
